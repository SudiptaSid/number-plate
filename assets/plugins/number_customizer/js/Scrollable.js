...
var Scrollable = new Class({
	...
	initialize: function(element, options) {
		...
		if (typeOf(element) == 'elements') {
			...
			this.container = new Element('div', {
				...
				name: this.element.get('id')
			}).inject(this.element.getParent(), 'bottom');

			var parent = this.element.getParent();
			var anchor = parent.getElement('.scrollBlock');
			var offset = anchor.getPosition(parent);

			this.container.setStyle('margin-top', offset.y.toInt());
	...
	reposition: function() {
		...
		(function() {
			...
			this.container.setStyle('height', this.size['height'].toInt()).setPosition({
		...
		if (this.options.proportional === true) {
			if (isNaN(this.options.proportionalMinHeight) || this.options.proportionalMinHeight <= 0) {
				...
				this.knob.setStyle('height', Math.max(knobHeight, minHeight).toInt());
			}
		}
